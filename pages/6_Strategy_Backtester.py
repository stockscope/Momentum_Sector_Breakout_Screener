...
import io # For capturing df.info() if needed for debugging
...
def calculate_indicators_for_date_slice(df_slice):
    if len(df_slice) < 20: return None # Min for 20D_High
    indicators = {}
...
# --- Backtesting Engine ---
@st.cache_data(ttl=timedelta(hours=1), show_spinner=False) # Cache backtest results for an hour
def run_backtest(
    all_hist_data, 
    tickers_to_test_tuple, # Expects a tuple for caching
    bt_start_date_input, 
    bt_end_date_input,   
    initial_capital,
    target_profit_pct,
    ...
    investment_per_trade_pct = 10
):
    tickers_to_test = list(tickers_to_test_tuple) # Convert back to list for iteration
    ...
    for ticker, ticker_df in all_hist_data.items():
        if not isinstance(ticker_df.index, pd.DatetimeIndex):
            all_hist_data[ticker].index = pd.to_datetime(ticker_df.index)
    ...
    for current_date in stqdm(backtest_date_range, desc="Backtesting Days"):
        current_portfolio_market_value_of_positions = 0 # Value of open positions at current price
        
        tickers_to_sell_today = []
        for ticker, pos_data in positions.items():
            if ticker not in all_hist_data or all_hist_data[ticker].empty: continue
            
            current_price = pos_data['entry_price'] # Default if no price found for today
            ...
            else: # Use last available price if current day's price is missing
        ...
        if len(positions) < max_concurrent_positions:
            ...
            for ticker in tickers_to_test:
                ...
                if inds is None or any(pd.isna(inds.get(k)) for k in ['Adj Close','50EMA', '20D_High', '52W_High']):
                    continue
                
                entry_signal = False
                adj_close_sig = inds['Adj Close']; ema50_sig = inds['50EMA']
                d20high_sig = inds['20D_High']; w52high_sig = inds['52W_High']
            ...
            for entry_candidate in potential_entries: # Simplified entry logic
                if len(positions) >= max_concurrent_positions: break
                ticker_to_buy = entry_candidate['ticker']
                actual_entry_price = entry_candidate['entry_price']
                
                current_total_value_for_sizing = cash + current_portfolio_market_value_of_positions 
                ...
                if cash >= amount_to_invest and amount_to_invest > 0 and actual_entry_price > 0:
                    ...
                    if quantity > 0:
                        ...
                        # current_portfolio_market_value_of_positions += cost_of_trade # Value of this new position is added at EOD calc
    ...
    for ticker, pos_data in positions.items():
        ...
        if not any(t['Ticker'] == ticker and t['Reason'] == 'End of Backtest' for t in trades_log): # Avoid duplicate EoB logs
            trades_log.append({
                'Ticker': ticker, 'Entry Date': pos_data['entry_date'], 'Entry Price': pos_data['entry_price'],
    ...
    if portfolio_history_daily:
        if portfolio_history_daily[-1]['Date'] != bt_end_date_actual:
         ...
         portfolio_history_daily.append({'Date': bt_start_date_ts, 'Portfolio_Value': initial_capital})
...
index_choice_bt = st.sidebar.selectbox("Index Universe for Backtest", ["NIFTY 500", "NIFTY 200"], key="bt_index_sidebar")
...
bt_start_date = st.sidebar.date_input("Start Date", value=st.session_state.bt_start_date, key="bt_start_date_picker")
bt_end_date = st.sidebar.date_input("End Date", value=st.session_state.bt_end_date, max_value=datetime.today() - timedelta(days=1), key="bt_end_date_picker")
st.session_state.bt_start_date = bt_start_date
st.session_state.bt_end_date = bt_end_date

initial_capital_bt = st.sidebar.number_input("Initial Capital", min_value=10000, value=100000, step=10000, key="bt_capital_input")
setup_to_backtest_bt = st.sidebar.selectbox(
    "Setup Type to Backtest", ["Breakout 52w", "Breakout", "Retest"], key="bt_setup_selectbox"
)
st.sidebar.markdown("--- Exit Strategy ---")
target_profit_pct_bt = st.sidebar.slider("Target Profit (%)", 1, 100, 20, 1, key="bt_tp_slider")
stop_loss_pct_bt = st.sidebar.slider("Stop Loss (%)", 1, 50, 10, 1, key="bt_sl_slider")
st.sidebar.markdown("--- Portfolio Allocation ---")
max_concurrent_positions_bt = st.sidebar.slider("Max Concurrent Open Positions", 1, 20, 5, 1, key="bt_max_pos_slider")
investment_per_trade_pct_bt = st.sidebar.slider("Investment per Trade (% of Portfolio)", 1, 50, 10, 1, key="bt_inv_pct_slider") # Max 50% per trade

run_backtest_button = st.sidebar.button("🚀 Run Backtest", type="primary", key="bt_run_button_main")
...
if run_backtest_button:
    if not bt_start_date or not bt_end_date or bt_start_date >= bt_end_date:
    ...
    elif initial_capital_bt <=0: st.error("Initial capital must be greater than zero.")
    else:
        ...
        if not tickers_for_backtest: st.error(f"Failed to load tickers for {index_choice_bt}.")
            ...
            if not all_hist_data_bt: st.error("Failed to fetch historical data for backtesting.")
            else:
                with st.spinner(f"Executing backtest for '{setup_to_backtest_bt}' on {index_choice_bt}... This may take some time."):
                    backtest_results_run = run_backtest( # Use a different variable name
                        all_hist_data_bt, tuple(tickers_for_backtest), bt_start_date, bt_end_date,
                ...
                st.session_state.backtest_results_data = backtest_results_run # Store in session state with a unique key
    ...
    if results:
        ...
        if df_trades is not None and not df_trades.empty:
            ...
            st.dataframe(df_trades.style.format({
                ...
                "Entry Date": "{:%Y-%m-%d}", "Exit Date": "{:%Y-%m-%d}" # Format dates
            }, na_rep="-"))
        elif results.get('num_trades', 0) == 0 : # Check if num_trades is explicitly 0
             st.info("No trades were executed during the backtest period with the given criteria.")
    else: st.error("Backtest execution failed or returned no results.")
else: st.info("Configure backtest parameters in the sidebar and click 'Run Backtest'.")
