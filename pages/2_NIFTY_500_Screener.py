...
def batch_download(tickers, start, end):
    ...
    for i in range(0, len(tickers), batch_size):
        ...
        try:
            batch_data = yf.download(batch, start=start, end=end, interval='1d', group_by='ticker', auto_adjust=False, progress=False)
            if isinstance(batch_data.columns, pd.MultiIndex):
                for ticker in batch:
                    if ticker in batch_data.columns.levels[0]:
                        all_data[ticker] = batch_data[ticker].copy()
            else:  # Single ticker
                all_data[batch[0]] = batch_data.copy()
        except Exception as e:
            print(f"Download failed for batch {batch}: {e}")
    return all_data
...
with st.spinner("🔍 Screening in progress... please wait"):
    ...
    if not data:
        st.error("❌ No data downloaded. Please check the internet or try again later.")
        st.stop()
